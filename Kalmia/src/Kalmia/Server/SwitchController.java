...
public class SwitchController extends Thread {
	...
	private LinkedList<CalendarListener> mylisteners = new LinkedList<CalendarListener>();// a list of calendars that
																							// this switch responds to
	...
	public SwitchController(SwitchRec rec, LinkedList<CalendarListener> allListeners, KalmiaISYClient isy, String hp) {
		...
		/*
		 * while(getStatus()==null){ try { sleep(5); } catch (InterruptedException e) {}
		 * }
		 */
	...
	public String getStatus() {
		...
		try {
			return (String) myISY.getCurrValue(myISY.getNodes().get(switchInfo.getSwitchAddress()),
		...
		// try {
		// return(String) myISY.getCurrValue(
		// myISY.getNodes().get(switchInfo.getSwitchAddress()),
		// InsteonConstants.DEVICE_STATUS);
		// } catch (NoDeviceException e) {
		// e.printStackTrace();
	...
	public String getSP() {
		if (switchInfo.getType().compareTo("thermostat") == 0) {
			try {
				if (heatOrAC) {
					return (String) myISY.getCurrValue(myISY.getNodes().get(switchInfo.getSwitchAddress()),
							InsteonConstants.CLIMATE_HEAT_SETPOINT);
				} else {
					return (String) myISY.getCurrValue(myISY.getNodes().get(switchInfo.getSwitchAddress()),
							InsteonConstants.CLIMATE_COOL_SETPOINT);
				}
			} catch (NoDeviceException e) {
				e.printStackTrace();
			}
			...
			try {
				if (heatOrAC) {
					Double tmp = new Double(
							(String) myISY.getCurrValue(myISY.getNodes().get(switchInfo.getSwitchAddress()),
									InsteonConstants.CLIMATE_HEAT_SETPOINT));
					tmp = tmp / 2;
					return tmp.toString();
				} else {
	...
	public String getHumidity() {
		if (switchInfo.getType().compareTo("thermostat") == 0) {
			try {
				return (String) myISY.getCurrValue(myISY.getNodes().get(switchInfo.getSwitchAddress()),
						InsteonConstants.CLIMATE_HUMIDITY);
			} catch (NoDeviceException e) {
				e.printStackTrace();
			} catch (NullPointerException e) {
			}
	...
	public String getUiThermoMode() {
		if (switchInfo.getType().compareTo("thermostat") == 0) {
			try {
				String tmp = (String) myISY.getCurrValue(myISY.getNodes().get(switchInfo.getSwitchAddress()),
						InsteonConstants.CLIMATE_MODE);
	...
	public void evaluateState() {
		...
		try {
			...
			for (int i = 0; i < mylisteners.size() && !occupied; i++) {
				try {
					List<Event> myEvents = mylisteners.get(i).getCurrentEvents();
					for (Event event : myEvents) {

						if (((DateTime.now().getValue() + lookAhead) > event.getStart().getDateTime().getValue())
								&& ((DateTime.now().getValue() - lookBack) < event.getEnd().getDateTime().getValue())) {
							occupied = true;
							break;
						}
					}
			...
			if ((switchInfo.getType().compareTo("circulator") == 0)
				...
				// Double.parseDouble(switchInfo.getCirculatorMin()));
			}
			...
			if ((switchInfo.getType().compareTo("circulator") == 0)
				...
				occupied = false;
				// System.out.println(temp + " " +
				// Double.parseDouble(switchInfo.getCirculatorMax()));
			}
			...
			System.err.println("weather isn't initialized yet");
		}
	}
	...
	private void logStatus() {
		...
		try {
			...
			logFile = new File(logPath + fileDateFormat.format(cal.getTime()) + " SwitchController "
					+ switchInfo.getTitle() + " log.csv");
	...
	public boolean turnSwitchOn() {
		...
		if (switchInfo.getType().compareTo("thermostat") == 0) {
			...
			System.out.println("Turn thermostat " + switchInfo.getTitle() + " at address '"
					+ switchInfo.getSwitchAddress() + "' ON");

			try {
				// if(getHeatSP().compareTo(switchInfo.getParams().get(0))!=0)
				while (!myISY.changeNodeState(CLISP_PROP, CLISP_VAL, switchInfo.getSwitchAddress())
						&& counter1 < timeout_limit && (getSP().compareTo(CLISP_VAL) != 0)) {
					try {
				...
				while (!myISY.changeNodeState("CLIMD", CLIMD_VAL, switchInfo.getSwitchAddress())
						&& counter2 < timeout_limit && (getThermoMode().compareTo(CLIMD_VAL) != 0)) {
					try {
			...
			System.out.println(
					"Turn " + switchInfo.getTitle() + " at address '" + switchInfo.getSwitchAddress() + "' ON");

			try {
				System.out.println("STATUS " + switchInfo.getTitle() + " " + getStatus());
				// if(getStatus().compareTo("DFON")!=0)
				while (!myISY.changeNodeState("DFON", null, switchInfo.getSwitchAddress()) && counter1 < timeout_limit
						&& (getStatus().compareTo("DFON") != 0)) {
					try {
	...
	public boolean turnSwitchOff() {
		...
		if (switchInfo.getType().compareTo("thermostat") == 0) {
			System.out.println("Turn thermostat " + switchInfo.getTitle() + " at address '"
					+ switchInfo.getSwitchAddress() + "' OFF");
			String CLISP_PROP, CLISP_VAL, CLIMD_VAL;
			...
			System.out.println(
					"Turn " + switchInfo.getTitle() + " at address '" + switchInfo.getSwitchAddress() + "' OFF");

			try {
				// if(getStatus().compareTo("DFOF")!=0)
				while (!myISY.changeNodeState("DFOF", null, switchInfo.getSwitchAddress()) && counter1 < timeout_limit
						&& (getStatus().compareTo("DFOF") != 0)) {
					try {
			...
			if (switchingMode) {
				try {
					...
					while (!myISY.changeNodeState("DFOF", null, switchInfo.getSwitchAddress())
							&& counter1 < timeout_limit && (getStatus().compareTo("0") != 0)) {
						try {
							System.out.println("Switch " + switchInfo.getTitle() + " is going to sleep.6");
							sleep(5000);
							counter1++;
